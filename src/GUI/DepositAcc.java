	...
	public DepositAcc() {
		...
		btnDeposit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// Validate input
				aacountNum = aacountNum == null ? "" : aacountNum.trim();
				if (aacountNum.isEmpty()) {
					JOptionPane.showMessageDialog(getComponent(0), "Please enter an account number");
					return;
				}
				try {
					amt = Double.parseDouble(textField_1.getText().trim());
				} catch (NumberFormatException nfe) {
					JOptionPane.showMessageDialog(getComponent(0), "Please enter a valid numeric amount");
					return;
				}
				...
				try {
						...
						// Persist changes
						FileIO.Write();
						// Show updated balance to the user and keep window open for more actions
						double newBal = 0.0;
						if (FileIO.bank.findAccount(aacountNum) != null) {
							newBal = FileIO.bank.findAccount(aacountNum).getbalance();
						}
						JOptionPane.showMessageDialog(getComponent(0), "Deposit successful. New balance: " + String.format("%.2f", newBal));
						// Clear only the amount field so user can perform another deposit quickly
